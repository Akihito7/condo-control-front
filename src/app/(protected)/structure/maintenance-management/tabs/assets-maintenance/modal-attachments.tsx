...
interface ModalAttachmentsProps {
  ...
  setAssetSelected: React.Dispatch<React.SetStateAction<Asset | null>>;
}
export function ModalAttachments({
  ...
  const { data: attachments, status } = useQuery({
    queryKey: ["attachments", assetSelected?.id],
    queryFn: () => fetchMaintenanceAssetsAttachments(assetSelected!.id),
    enabled: !!assetSelected?.id,
  });
  ...
  const { mutateAsync: handleDeleteAttchment } = useMutation({
    mutationFn: (attachmentId: number) =>
      deleteMaintenanceAssetsAttachment(attachmentId),
    onSuccess: () => {
  ...
  const { mutateAsync: addAttachments } = useMutation({
    mutationFn: (form: FormData) => addAttachmentMaintenanceAsset(form),
    onSuccess: () => {
      query.invalidateQueries({
        queryKey: ["attachments", assetSelected?.id],
      });
  ...
  async function handleAddAttachments(files: File[]) {
    ...
    form.append("maintenanceAssetId", String(assetSelected?.id));
