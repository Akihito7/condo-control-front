...
export function useMaintenances({ dateFormatted }: { dateFormatted: string }) {
  ...
  const {
    data: maintenancesStatusOptions,
    status: maintenancesStatusOptionsStatus,
  } = useQuery({
    queryKey: ["maintenances", "status-options"],
    queryFn: fetchMaintenancesStatus,
    enabled: !!condominiumId,
  });

  const { data: assets, status: assetsStatus } = useQuery({
    queryKey: ["maintenances", "assets", user.id],
  ...
  return {
    ...
    maintenancesStatusOptions: maintenancesStatusOptions?.filter((status) =>
      STATUS_ALLOWS.includes(status.id),
    ),
